    ...
    const fetchAuctions = async () => {
      ...
      try {
        ...
        const result = await readContract({
          address: VITE_AUCTION_CONTRACT_ADDRESS as `0x${string}`,
        ...
        if (result && Array.isArray(result)) {
          ...
          const auctionSummaries: AuctionSummary[] = auctionAddresses.map(auctionAddress => ({
            address: auctionAddress,
            isUserAuction: address ? address.toLowerCase() === auctionAddress.toLowerCase() : false
          }));
