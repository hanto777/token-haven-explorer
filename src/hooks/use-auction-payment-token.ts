...
export const useAuctionPaymentToken = () => {
  ...
  useEffect(() => {
    async function populate() {
      ...
      const contractSymbol = await readContract(wagmiConfig, {
        address: address as `0x${string}`,
        abi: confidentialErc20Abi,
        functionName: "symbol",
      });
      if (typeof contractSymbol === 'string') setPaymentTokenSymbol(contractSymbol);

    }
    if (!context.tokens) return;
    const auctionToken = context.tokens.find(({ isAuctionPaymentToken }) => !!isAuctionPaymentToken)
