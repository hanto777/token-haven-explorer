...
const SwapToFromSection: React.FC<SwapToFromSectionProps> = ({
  ...
  return (
    <Card
      className={`${swapFrom ? 'bg-card' : 'bg-card-accent'} p-8 border-none`}
    >
          ...
          <div className="relative ">
            {swapFrom ? (
              <Input
                ...
                className="p-0 pr-8 border-none bg-card md:text-4xl"
                step="any"
              />
            ) : (
              <div className="p-0 pr-8 border-none md:text-4xl">
                {amount ? amount : '0.0'}
              </div>
            )}
          </div>
