...
export const TokenProvider = ({ children }: { children: ReactNode }) => {
  ...
  const decryptToken = (id: string, decryptedBalance: string) => {
    const updatedTokens = tokens.map((token) => {
      if (token.id === id && token.isEncrypted) {
        if (token.isConfidential) {
          // const value =
          //   parseFloat(decryptedBalance) *
          //   getTokenPrice(token.symbol, chainId);
          const value = 20; // or calculate real value if needed
          return {
            ...token,
            isDecrypted: true,
            balance: "23",
  ...
  const sendToken = async (
    ...
    try {
      ...
      toast.success("Transfer successful", {
        description: `Sent ${amount} ${token.symbol} to ${to.slice(
          0,
          6
        )}...${to.slice(-4)}`,
      });
